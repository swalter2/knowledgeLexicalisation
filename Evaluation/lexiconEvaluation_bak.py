...
def calculateAccuracy(user_lex, gold, subject_map, object_map):
    entries_counter = 0
    points = 0
    if user_lex.getCanonicalForm().lower() == gold.getCanonicalForm().lower() and MatchArrays(user_lex.getSense(), gold.getSense()) == True and user_lex.getPartOfSpeech() == gold.getPartOfSpeech(): #print str(user_lex.getCanonicalForm())
        local_points = 0
        entries_counter += 1
    ...
    for uri_to_compare_with in hm_of_uris:
        ...
        for user_lex in user_lexicon:
            for gold in lexicon_entries_gold:
                ...
                entries_counter_local, points_local = calculateAccuracy(user_lex, gold, subject_map, object_map)
                entries_counter += entries_counter_local
                points += points_local

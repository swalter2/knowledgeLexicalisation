...
class MaltParser(ParserI):
    ...
    def tagged_parse(self, sentence, verbose=False):
        ...
        try:
            ...
            f.write('\n')
            f.close()
            cmd = ['java', '-Xmx4096m -jar %s' % self._malt_bin,'-w %s' %self.working_dir,'-c %s' % self.mco, '-i %s' % input_file, '-o %s' % output_file, '-m parse']
    ...
    def tagged_parse_many_sentences(self, list_sentence, verbose=False):
        """
        Use MaltParser to parse a sentence. Takes a sentence as a list of
        (word, tag) tuples; the sentence must have already been tokenized and
        tagged.

        :param sentence: Input list of sentences to parse
